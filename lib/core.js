...
let printerObject = {
  ...
  init: function(initConfig){
    ...
    this.iface = getInterface(initConfig.interface);
    ...
    if(initConfig.extraSpecialCharacters) config.specialCharacters = this.mergeObjects(config.specialCharacters, initConfig.extraSpecialCharacters);
  ...
  execute: function(cb){
    this.iface.execute(buffer, function (err) {
      if (!err) {
        buffer = null;
  ...
  cut: function(){
    this.append(config.CTL_VT);
    this.append(config.CTL_VT);
    this.append(config.PAPER_FULL_CUT);
    this.append(config.HW_INIT);
  },
  ...
  partialCut: function(){
    this.append(config.CTL_VT);
    this.append(config.CTL_VT);
    this.append(config.PAPER_PART_CUT);
    this.append(config.HW_INIT);
  },


  beep: function(){
    if (this.printerConfig.type === printerTypes.STAR){
      console.error("Beep not supported on STAR yet!");
    } else {
      this.append(config.BEEP);
    }
  },
  ...
  bold: function(enabled){
    if(enabled) this.append(config.TXT_BOLD_ON);
    else this.append(config.TXT_BOLD_OFF);
  },


  underline: function(enabled){
    if(enabled) this.append(config.TXT_UNDERL_ON);
    else this.append(config.TXT_UNDERL_OFF);
  },


  underlineThick: function(enabled){
    if(enabled) this.append(config.TXT_UNDERL2_ON);
    else this.append(config.TXT_UNDERL_OFF);
  },


  upsideDown: function(enabled){
     if(enabled) this.append(config.UPSIDE_DOWN_ON);
     else this.append(config.UPSIDE_DOWN_OFF);
  },


  invert: function(enabled){
    if(enabled) this.append(config.TXT_INVERT_ON);
    else this.append(config.TXT_INVERT_OFF);
  },


  openCashDrawer: function(){
    if(this.printerConfig.type === printerTypes.STAR){
      this.append(config.CD_KICK);
    } else {
      this.append(config.CD_KICK_2);
      this.append(config.CD_KICK_5);
    }
  },


  alignCenter: function (){
    this.append(config.TXT_ALIGN_CT);
  },


  alignLeft: function (){
    this.append(config.TXT_ALIGN_LT);
  },


  alignRight: function(){
    this.append(config.TXT_ALIGN_RT);
  },


  setTypeFontA: function(){
    this.append(config.TXT_FONT_A);
  },


  setTypeFontB: function(){
    this.append(config.TXT_FONT_B);
  },


  setTextNormal: function(){
    this.append(config.TXT_NORMAL);
  },


  setTextDoubleHeight: function(){
    this.append(config.TXT_2HEIGHT);
  },


  setTextDoubleWidth: function(){
    this.append(config.TXT_2WIDTH);
  },

  setTextQuadArea: function(){
    this.append(config.TXT_4SQUARE);
  },


  newLine: function(){
    this.append(config.CTL_LF);
  },
  ...
  isPrinterConnected: function(exists){
    this.iface.isPrinterConnected(exists);
  },
  ...
  printQR: function(str, settings){
    if (this.printerConfig.type === printerTypes.STAR) {
      this.append(star.printQR(str, settings));
    } else if(this.printerConfig.type === printerTypes.EPSON) {
      this.append(epson.printQR(str, settings));
    } else {
      console.error("QR not supported on '" + this.printerConfig.type + "' yet!");
    }
  },
  ...
  printBarcode: function(data, type, settings){
    if(this.printerConfig.type === printerTypes.EPSON) {
      this.append(epson.printBarcode(data, type, settings));
    } else {
      console.error("Barcode not supported on '" + this.printerConfig.type + "' yet!");
    }
  },
  ...
  maxiCode: function(data, settings){
    if(this.printerConfig.type === printerTypes.EPSON) {
      this.append(epson.maxiCode(data, settings));
    } else {
      console.error("MaxiCode not supported on '" + this.printerConfig.type + "' yet!");
    }
  },
  ...
  code128: function(data, settings) {
    if (this.printerConfig.type === printerTypes.STAR) {
      this.append(star.code128(data, settings));
    } else {
      console.error("Code128 not supported on '" + this.printerConfig.type + "' yet!");
    }
  },
  ...
  pdf417: function(data, settings) {
    if (this.printerConfig.type === printerTypes.STAR) {
      this.append(star.pdf417(data, settings));
    } else if(this.printerConfig.type === printerTypes.EPSON) {
      this.append(epson.pdf417(data, settings));
    } else {
      console.error("PDF417 not supported on '" + this.printerConfig.type + "' yet!");
    }
  },
    ...
    try {
      ...
      if(image.slice(-4) === ".png"){
        if (this.printerConfig.type === printerTypes.STAR){
          ...
          console.error("Image print not supported on '" + this.printerConfig.type + "' yet!");
        }
      } else {
  ...
  raw: function(text, cb) {
    this.iface.execute(text, function (err) {
      if (!err) {
        if (typeof cb === "function") {
          cb(null);
        }
  ...
  append: function(buff){
    ...
    if(!buffer && this.printerConfig.characterSet) buffer = this.setInternationalCharacterSet(this.printerConfig.characterSet);
  ...
  setInternationalCharacterSet: function(charSet){
